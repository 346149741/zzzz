...
export function calculationHeading(startPoint, endPoint) {
...
export function Cartesian3_to_WGS84(point) {
  ...
  const coordSwitchFuc = CoordsTransform.gcj02_wgs
  const res = coordSwitchFuc(lng, lat)
  return {
    lnglat: [res[0].toFixed(4), res[1].toFixed(4)],
    lnglathei: {
      lon: res[0].toFixed(4),
      lat: res[1].toFixed(4),
      hei: alt.toFixed(4)
...
// 计算多边形面积
export function getArea(points, positions) {
  var res = 0
  // 拆分三角曲面
 ...
 *@return:
 */
export function pointToGeoJson(points) {
  var geojsonobj = {
    'type': 'Feature',
    'geometry': {
      'type': 'Point',
 ...
 *@return:
 */
export function lineToGeoJson(points) {
  var geojsonobj = {
    'type': 'Feature',
    'geometry': {
      'type': 'LineString',
 ...
 *@return:
 */
export function polygonToGeojson(points) {
  var geojsonobj = {
    'type': 'Feature',
    'geometry': {
      'type': 'Polygon',
...
/*
 *@Description: 获取多个点的中心点，使用turf获取的是geojson格式，转为坐标数组，
 *@paramter:[[117.42,34.12],[116.64,42.13]..]
 *@return:[117.54,45.36]
 */
export function getPointsCenter(points) {
  var featureCollection = []
  for (var i = 0; i < points.length; i++) {
    featureCollection.push(turf.point([points[i][0], points[i][1]]))
  }
...
// 获取坐标
export function getcartesian(position, viewer) {
  var pickedObject = viewer.scene.pick(position)
  // 场景坐标
  if (pickedObject && Cesium.defined(pickedObject.primitive) && pickedObject.content) {
...
export function getcartesian2(position, viewer) {
  var pickedObject = viewer.scene.pick(position)
  // 场景坐标
