    ...
    <div class="app-container">
      <el-card class="box-card app-body">
        <div slot="header" class="clearfix">
          <div class="header-title" style="display: flex;justify-content: space-between"><span style="cursor:pointer;" @click="$router.go(-1)">返回</span><span>多媒体资源</span><span /></div>
        </div>
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane v-for="item in list" :label="item.type" :name="item.name">
            <el-row :gutter="20">
              <el-col v-for="(cell,index) in item.children" :key="index" :span="4" class="item">
                <el-card class="item-card">
                  ...
                  <div class="txt tac">{{ cell.score===0?'免费':`${cell.score}积分` }}</div>
                </el-card>
              </el-col>
            </el-row>
          </el-tab-pane>
...
export default {
  ...
  methods: {
    handeOpen() {
      this.$router.push({ name: 'SelfLearningMediaVoiceDetail' })
    }
  }
}
</script>
