...
import RemoteDoorControlDialog from '@/components/DialogCmpts/dialogs/RemoteDoorControlDialog'
...
export default class GateControl {
  ...
  initFloors(arr, detailCallback) {
    const _that = this
    const res = []
    arr.forEach((item) => {
      if (item.location && item.location.constructor.name === 'String') {
        item.location = JSON.parse(item.location)
      }
      const geom = new Polygon([item.location])
      geom.transform('EPSG:4326', 'EPSG:3857')
      const fea = new Feature({
        geometry: geom,
        info: item
      ...
      fea.on('showDetailInfo', e => {
        const info = e.target.get('info')
        const detailComp = new RemoteDoorControlDialog({
          title: info.towerLocation
        })
      })
      res.push(fea)
    })
