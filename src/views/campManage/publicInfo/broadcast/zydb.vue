<template>
  <div>
    ...
    >
      ...
      <el-table-column label="点播人" fixed="left" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.dbr }}</span>
        </template>
      </el-table-column>

      <el-table-column label="所在单位" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.szdw }}</span>
        </template>
      </el-table-column>
      <el-table-column label="点播内容" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.dbnr }}</span>
        </template>
      </el-table-column>
      <el-table-column label="点播类型" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.dblx }}</span>
        </template>
      </el-table-column>
      <el-table-column label="点播来源" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.dbly }}</span>
        </template>
      </el-table-column>
      <el-table-column label="点播时间" align="center"> 
        <template slot-scope="scope">
          <span>{{ scope.row.dbsj }}</span>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
      ...
      >
        <template slot-scope="scope">
          <div class="t-col-operate">
            <el-button v-if="scope.row.zxzt" type="primary" size="mini" @click="startClick(scope.row)">执行</el-button>
            <el-button v-if="!scope.row.zxzt" type="primary" size="mini" @click="cancleClick(scope.row)">取消</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      list: [{ dbr: '彭万里', szdw: '探鹰一营', dbnr: '给新兵连的弟兄们打打气', dblx: '口播祝福语', dbly: '公众号留言', dbsj: '2020-02-02 20:20:20' },
        { dbr: '高大山', szdw: '追鹰二营', dbnr: '营长的生日就要到了，点播一首', dblx: '播放歌曲', dbly: '电话', dbsj: '2020-02-02 20:20:20' },
        { dbr: '谢大海', szdw: '探鹰三营', dbnr: '给新兵连的弟兄们打打气', dblx: '口播祝福语', dbly: '私信', dbsj: '2020-02-02 20:20:20' }],
      total: null,
      checkdetail: false,
      listQuery: {
        pageNo: 1,
